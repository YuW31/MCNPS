...
setwd("/Users/yuwang/Desktop/Research/MC-NPS/nps/sim2_pilot_300")
...
for (YY in 1:nrow(Matrix.con)) {
  setwd(paste0("/Users/yuwang/Desktop/Research/MC-NPS/nps/sim2_pilot_300/Con",YY))
  cat("Condition:", YY,"\n")
  ...
  if(length(fit_mcdina$item)==0){
    ...
    for (i in 1:N){
      ...
      i.est.plot <- cbind(1:L,i.est.plot, t(dat))
      i.est.plot <- data.frame(i.est.plot)
      colnames(i.est.plot) <- c("Cycle","Membership","X")
      p1 = ggplot(i.est.plot, aes(x = Cycle, y = Membership,label = X)) +
        geom_point(size = 3, color = "blue") +
  ...
  plot.df = data.frame(rbind(process.par.all_opmc, process.par.all_opb, process.par.all_noop,
                             process.par.all_gnps, process.par.all_jsd, process.par.all_r))
