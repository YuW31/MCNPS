### employed op for JSD
setwd("/Users/yuwang/Desktop/Research/MC-NPS/nps/sim2_JSDop")
...
for (YY in 2:nrow(Matrix.con)) {
  # setwd(paste0("/Users/yuwang/Desktop/Research/MC-NPS/nps/sim2_JSDop/Con",YY))
  cat("Condition:", YY,"\n")
  ...
  if(length(fit_mcdina$item)==0){
    ...
    if (H == 4){
      ...
      for (i in 1:N) {
        ...
        for (j in 1:J_opmc){
          # administer.J <- Q.optimal.binary(q.matrix,obs.x,key.all)
          mc.administer.J <- Q.optimal.mc(mc.q,obs.x,key.all,save.m,H)
          obs.x <- rbind(obs.x,c(mc.administer.J,x.n.bank[,mc.administer.J]))
          ini.J <- obs.x[,1]
          jsd.est <- JSD(mc.q,save.m,H,p.eta = fit_mcdina$item,x.n.bank[,ini.J,drop=FALSE],ini.J,key.all)
          process.est.all_JSD[[j]][i,] = LatentClass[jsd.est$Est,]
        }
        
        administer.J = c(ini.J) # removed items
        
        for (j in (J_opmc+1):(L+1)){  #+1 for estimation, not actually administered
          # jsd.selection = JSD(mc.q,save.m,O,p.eta = 0.82,dat,administer.J,key) 
          jsd.selection = JSD(mc.q,save.m,H,p.eta = fit_mcdina$item,x.n.bank[,administer.J,drop=FALSE],administer.J,key.all)
          process.est.all_JSD[[j-1]][i,] = LatentClass[jsd.selection$Est,]
  ...
  write.csv(plot.df,paste0("result_sim2.1_con", YY,".csv"),row.names=FALSE)
  ...
  ggsave(paste0("result_sim2.1_con", YY,".png"),p.list[[YY]],width = 10, height = 8) # 8*8, if 3K
  
  
  # ### class size
  # n.class <- subset(PAR.class(true.att,process.est.all_opmc[[1]]),TRUE,ClassSize)
  # write.csv(n.class,paste0("result_sim2.1_con", YY,"_classN",".csv"))
  # 
  ...
  #     
  #     write.csv(plot.df,paste0("result_sim2.1_con", YY,"_class",m,".csv"),row.names=FALSE)
  #     
  #     if(length(fit_mcdina$item)==0){
  #       plot.df = subset(plot.df, Method != "JSD")
  #       colors = colors[-2]
  ...
  #       scale_shape_manual(values = shapes)
  #     
  #     ggsave(paste0("result_sim2.1_con", YY,"_class",m,".png"),p,width = 10, height = 8) # 8*8, if 3K
  #   }
  #   
  # }
  # 
  ...
  # write.csv(n.classK,paste0("result_sim2.1_con", YY,"_classKN",".csv"))
  # 
  # 
  # for (m in 0:K){
  #   colors = c("MC-NPS" = "#000000", "JSD" = "#E69F00", "Random" = "#56B4E9", "GNPS" = "#009E73")
  ...
  #     plot.df$Method = factor(plot.df$Method, levels = c("MC-NPS","JSD","Random","GNPS"))
  #     
  #     write.csv(plot.df,paste0("result_sim2.1_con", YY,"_classK",m,".csv"),row.names=FALSE)
  #     
  #     if(length(fit_mcdina$item)==0){
  #       plot.df = subset(plot.df, Method != "JSD")
  #       colors = colors[-2]
  ...
  #       scale_shape_manual(values = shapes)
  #     
  #     ggsave(paste0("result_sim2.1_con", YY,"_classK",m,".png"),p,width = 10, height = 8) # 8*8, if 3K
  #   }
  #   
  # }
