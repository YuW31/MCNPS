...
for (YY in 1:nrow(Matrix.con)) {
  ...
  plot.df = read.csv(paste0("result_simTRUE_con", YY,".csv"))
  
  plot.df$Method = factor(plot.df$Method, levels = c("MC-NPS","JSD","Random"))
  levels(plot.df$Method) <- c("MC-NPS","JSD_True","Random")
  
  colors = c("MC-NPS" = "#D55E00", "JSD_True" = "#0072B2", "Random" = "#E69F00")
  shapes = c("MC-NPS" = 15, "JSD_True" = 16, "Random" = 17)
...
ggsave(paste0("result_simTRUE.png"),ag,width = 20, height = 14)
