### How about if the first 2/4 items are perfectly aligned with the ideal response?
### Then, observe the performance of Delta method
...
for (m in 2:nrow(LatentClass)){
  cat("LatentClass:", m)
  # sapply(1:16,function(x){dir.create(paste0("m",x))})
  setwd(paste0("/Users/yuwang/Desktop/Research/MC-NPS/nps/discrimination2/m",m))
  ...
  process.est.all <- vector(length = L-J_star+1,mode = "list")
  ...
  for (i in 1:N) {
    ...
    for (j in 1:(L-J_star)) {
      ...
      # below lines are merged into MCNPS()
    ...
    i.est.plot <- cbind(J_star:L,i.est.plot)
    ...
    p1 = ggplot(i.est.plot, aes(x = Cycle, y = Membership)) +
      ...
      scale_x_continuous(breaks = seq(2, L, by = 1)) + 
